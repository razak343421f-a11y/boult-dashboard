...
// ========== HELPER FUNCTIONS ==========
function buildDateQuery(dateStr,attributeName){if(!dateStr)return[];const start=new Date(dateStr);const end=new Date(dateStr);end.setHours(23,59,59,999);return[Appwrite.Query.greaterThanEqual(attributeName,start.toISOString()),Appwrite.Query.lessThanEqual(attributeName,end.toISOString())];}
function formatDateTime(iso){if(!iso)return'-';const d=new Date(iso);return d.toLocaleDateString()+' '+d.toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit'});}
